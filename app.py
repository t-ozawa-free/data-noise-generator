    ...
    try:
        ...
        if st.button("🔄 プレビュー生成", type="primary"):  # プレビュー生成ボタン
            ...
            with st.expander("📊 列ごとの欠損率を表示"):
                ...
                # 欠損数でフィルタを行う
                missing_by_col_filtered = missing_by_col[missing_by_col > 0]

                # その後DataFrameを作成（文字列に変換）
                missing_rate_df = pd.DataFrame({
                    "列名": missing_by_col_filtered.index,
                    "欠損数": missing_by_col_filtered.values,
                    "欠損率": [f"{rate:.2f}%" for rate in (missing_by_col_filtered / len(df) * 100).values]
                })
                if len(missing_rate_df) > 0:
                    st.dataframe(missing_rate_df, width=True)
                else:
